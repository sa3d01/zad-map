...
class CartCollection extends ResourceCollection
{
    public function toArray($request): array
    {
        ...
        foreach ($this as $obj) {
            ...
            $arr['provider']['name'] = $product->user->provider->name;
            $arr['name'] = $product->name;
            $arr['image'] = $product->images[0];
            $arr['price'] = (double)$product->price;
            $arr['delivery_price'] = (double)$product->user->provider->delivery_price;
            $arr['has_delivery'] = (double)$product->user->provider->has_delivery;
            $arr['in_cart'] = true;
